...
var Magneticmediajs = (function () {
    ...
    var _makeKeyboard = function (event) {
        event.stopPropagation();
        var imageSelected = event.data.param1.find('img');
        switch (event.which) {
            case 32: // Spacebar
                event.preventDefault();
        ...
        for (var k = 0, len = urlWithQuery.length; k < len; k++) {
            ...
            if (splitter[0] === 'magnetid') {
                if (gallery && splitter[1] === elmentid) {
                    $thisContainer.
                        find('img.mjsThumb' + splitter[1].substr(splitter[1].length - 1)).
                        trigger('click');
                } else if (!gallery &&
    ...
    Magneticmediajs.prototype.init = function () {
        ...
        for (var i = 0, len = elements.length; i < len; i++) {
            ...
            for (var j = 0; j < this.nbOfData; j++) {
                ...
                if (gallery) {
                    ...
                    if (this.ecommerce) {
                        if (j === 0) {
                            ...
                            $thisContainer.find('#mjs-ecommerce-col').append('<div class="mjs-generic ' +
                    ...
                    $thumb.on('error', {param: j}, function (event) {
                        ...
                        $thisContainer.find('.mjsThumb' + number).parent().remove();
                        thisT.loadErrors = thisT.loadErrors + 1;
                    });
                    $thumb.on('load', function () {
                        if ($(this).attr('src') !==
                            'data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs%3D') {
                            thisT.thumbLoadCount = thisT.thumbLoadCount + 1;
                        }
                    });
                    if (data[j][0] === 'youtube') {
                    ...
                    $thisContainer.find('.mjs-video-play-gallery').css({'color': '#' + color2});
                ...
                $tempSelector.on('click', {param: j}, function (event) {
                    event.stopPropagation();
                    var $body = $('body');
                    // bug fix #1, when zoom on = no scroll
                    if (zoomOn) {
                        mjsGlobal.orgOverflow = $body.css('overflow');
                        $body.css('overflow', 'hidden');
                    }
                    // prevent too much click from users 
                    if ($body.has('.mjs-fullscreen').length) {
                        return;
                    }
                    ...
                    if (thisT.nbOfData > 1 && gallery) {
                        ...
                        $previous.on('click', function () {
                            ...
                            imageOnNumber--;
                            if (imageOnNumber === -1) {
                                imageOnNumber = thisT.nbOfData - 1;
                            }
                        ...
                        $next.on('click', function () {
                            ...
                            $wrapper.off('mouseleave mouseenter');
                            imageOnNumber++;
                            if (imageOnNumber === thisT.nbOfData) {
                                imageOnNumber = 0;
                            }
                            $('.mjs-title-area').remove();
                            if (displayTitle) {
                    ...
                    $fullscreen.on('click', function (event) {
                        if (!mjsGlobal['timeSpacer']) {
                            ...
                            // make body scrollbar visible again for zoom
                            if (zoomOn) {
                                $body.css('overflow', mjsGlobal.orgOverflow);
                            }
                        } else {
                            event.stopPropagation();
                            event.preventDefault();
                ...
                // Add deeplinking
                _mjsDirectAccess(tempSelectorID, gallery, $thisContainer);
            ...
            if (gallery) {
                var thumbTimer = setInterval(function ($thisContainer) {
                    if (thisT.thumbLoadCount >= thisT.nbOfData - thisT.loadErrors) {
                        $thisContainer.show(0, function () {
                            if (thisT.ecommerce) {
                                var colHeight = $thisContainer.find('#mjs-ecommerce-col').height();
                                if (colHeight > $thisContainer.find('.mjs-ecommerce-main').height()) {
                                    $thisContainer.height(colHeight);
                                    $thisContainer.find('.mjs-ecommerce-main').
                                        addClass('mjs-ecommerce-main-adjust');
                                }
                            }
                        });
        ...
        if (thisT.ecommerce) {
            $(window).on('resize', function () {
                ...
                if (colHeight > $thisContainer.find('.mjs-ecommerce-main').height()) {
                    $thisContainer.height(colHeight);
                    $thisContainer.find('.mjs-ecommerce-main').addClass('mjs-ecommerce-main-adjust');
                } else {
                    $thisContainer.find('.mjs-ecommerce-main').removeClass('mjs-ecommerce-main-adjust');
                }
            });
        }
